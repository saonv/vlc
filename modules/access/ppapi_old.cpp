...
static int32_t new_download(access_sys_t* sys, const size_t from_meta_index) {
...

  {
    ...
    msg_Err(sys->access, "sending headers: `%s`", headers);
  ...
  PP_Bool success;
  do {
    success = irequest->SetProperty(request, PP_URLREQUESTPROPERTY_METHOD, vlc_ppapi_mk_str(&http_method));
    if (success != PP_TRUE)
      break;
    success = irequest->SetProperty(request,
...
static void log_response(access_t *access, const char *header, size_t header_len){
  size_t pos = 0, i = 0;
  char *headerItem = (char *)malloc(512);
  for (; i < header_len; i++){
    if (header[i] == '\r' || header[i] == '\n'){
      if (i - pos > 0){
        memset(headerItem, 0, 512);
        memcpy(headerItem, header + pos, i - pos);
        msg_Err(access, "Response Header: %s", headerItem);
      }
      pos = i;
    }
  }
  ...
  msg_Err(access, "server response to HEAD is missing the Content-Length header: %i, %s", header_len, header);
  free(headerItem);
}
...
static int discover_file_size(access_t* access, PP_Instance instance, const char* url, uint64_t *file_size) {
  ...
  if(success != PP_TRUE) { 
    msg_Err(access, "couldn't SetProperty");
    goto error; 
  }

  code = iloader->Open(loader, request, PP_BlockUntilComplete());
  if(code != PP_OK) { 
    msg_Err(access, "couldn't Open `%i`", code);    
    goto error; 
  }

  response = iloader->GetResponseInfo(loader);
  if(response == 0) { 
    msg_Err(access, "couldn't GetResponseInfo");    
    goto error; 
  }
  ...
  if(!found || sscanf(cl + sizeof(content_length) - 1, ": %llu", file_size) <= 0) {
    log_response(access, header_str, header_len);
    goto error;
  }
